...
interface ViewRubricsModalProps {
  isOpen: boolean;
  onClose: () => void;
  data?: any;
}

const ViewRubricsModal: React.FC<ViewRubricsModalProps> = ({ isOpen, onClose, data }) => {
  const rubricsHeader = {
  ...
  const handleExportPDF = () => {
    ...
    doc.text("MTE Rubrics Report", 14, 20);
    ...
    doc.save(`Rubrics_${rubricsHeader.course.replace(/\s+/g, "_")}.pdf`);
  };
          ...
          >
            <div className="inline-block w-full max-w-6xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white border border-gray-400 rounded-lg shadow-xl relative font-sans">
              ...
              <div className="bg-[#1f2937] px-6 py-3 flex justify-between items-center text-white border-b border-gray-600 font-sans">
                <span className="text-base font-semibold uppercase tracking-wide">Rubrics Details</span>
